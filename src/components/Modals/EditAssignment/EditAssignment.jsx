...
import editAssignment from "../../../store/actions/editAssignment";
...
const EditAssignment = ({ assignment }) => {
  ...
  useEffect(() => {
    if (assignment.id) {
      ...
      for (let i = 0; i < assignment.files.length; i++) {
        const element = assignment.files[i];
        files_name.push(element.name);
      }
      let { files, ...newassignment } = assignment;
      setState({ ...state, ...newassignment, files_name });
    }
  }, [assignment]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log(state);
    dispatch(editAssignment(state, setState));
  };
