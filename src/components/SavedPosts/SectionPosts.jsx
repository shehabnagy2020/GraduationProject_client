...
const SectionPosts = () => {
  const { savedPostsArr, userDetails, pageHelpers } = useSelector(
    (state) => state
  );
  ...
  const fetchData = (_) => {
    dispatch(getPost(pageHelpers.pageSaved));
  };
  ...
  return (
    <div className="home-posts-container">
      <InfiniteScroll
        dataLength={savedPostsArr.data.length} //This is important field to render the next data
        next={fetchData}
        hasMore={savedPostsArr.hasMore}
      >
        {savedPostsArr.data.length >= 1 ? (
          savedPostsArr.data.map((item, index) => (
            <div key={index} className="home-post">
              ...
              <div className="post-footer">
                ...
                <button
                  ...
                  onClick={(_) => dispatch(toggleSavePost(item.id, "saved"))}
                >
